...
const features = [
  { icon: Users, title: "Mentorship", desc: "Get guidance from experienced alumni in your field of interest." },
  { icon: Briefcase, title: "Career Support", desc: "Access job referrals, resume reviews, and career advice." },
  { icon: Calendar, title: "Events", desc: "Attend reunions, workshops, and networking meetups." },
  { icon: Network, title: "Networking", desc: "Build meaningful connections with alumni across industries." },
];
...
const FeaturesSection = () => {
  return (
    <section id="features" className="bg-muted py-10 px-4">
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="grid grid-cols-2 gap-3">
          {features.map((f) => (
            <div key={f.title} className="bg-card rounded-lg p-4 shadow-sm border border-border flex flex-col items-start gap-2">
