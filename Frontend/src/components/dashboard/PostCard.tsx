...
const PostCard = ({ post }: PostCardProps) => {
  ...
  const deletePost = async () => {
    ...
    const msg = isOwner ? "Delete this post?" : "Delete this post as admin?";
    if (!confirm(msg)) return;
    try {
      await postsApi.delete(post.id);
      queryClient.invalidateQueries({ queryKey: ["posts"] });
      toast.success(isOwner ? "Post deleted" : "Post removed by admin");
    } catch (err) {
    ...
    <div className="glass-card rounded-2xl overflow-hidden hover-lift">
      ...
      <div className="flex items-center gap-3 p-4 pb-0">
        ...
        {canDeletePost && (
          <button
            onClick={deletePost}
            title={isOwner ? "Delete post" : "Delete as admin"}
          ...
          >
            {isOwner ? <Trash2 className="w-4 h-4" /> : <Shield className="w-4 h-4" />}
          </button>
        )}
      </div>
