...
//   DELETE /api/jobs/:id           — delete own job (admins use /api/admin/jobs/:id)
//   POST   /api/jobs/:id/toggle    — toggle is_open (own job)
...
router.put('/:id', protect, async (req, res) => {
  try {
    ...
    if (existing[0].posted_by !== req.user.id) return res.status(403).json({ error: 'Forbidden' });
...
router.delete('/:id', protect, async (req, res) => {
  try {
    ...
    if (rows[0].posted_by !== req.user.id) return res.status(403).json({ error: 'Forbidden' });
