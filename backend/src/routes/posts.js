...
// DELETE /api/posts/:id  — author only (admins use /api/admin/posts/:id)
...
// DELETE /api/posts/:postId/comments/:commentId — author of comment OR admin
router.delete('/:postId/comments/:commentId', protect, async (req, res) => {
  try {
    ...
    if (rows[0].user_id !== req.user.id) {
      const adm = await db.query('SELECT is_admin, is_super_admin FROM users WHERE id = $1', [req.user.id]);
      isAdmin = !!adm.rows[0]?.is_admin || !!adm.rows[0]?.is_super_admin;
