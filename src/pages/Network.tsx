...
const Network = () => {
  ...
  const getStatus = (userId: string) => {
    const conn = connections.find(
      (c) => (c.requester_id === user!.id && c.addressee_id === userId) || (c.addressee_id === user!.id && c.requester_id === userId)
    );
    if (!conn) return "none";
  ...
  const acceptRequest = async (userId: string) => {
    await supabase.from("connections").update({ status: "accepted" }).eq("requester_id", userId).eq("addressee_id", user!.id);
            ...
            return (
              <div key={profile.id} className="bg-card rounded-xl shadow p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm text-foreground truncate">{profile.full_name || "Alumni"}</p>
                  <p className="text-xs text-muted-foreground truncate">{profile.headline || ""}</p>
                  {profile.resume_url && (
                    <a href={profile.resume_url} target="_blank" rel="noopener noreferrer" className="text-xs text-secondary hover:underline">View Resume</a>
                  )}
                </div>
                <div>
                  {status === "none" && (
                    <button onClick={() => sendRequest(profile.user_id)} className="flex items-center gap-1 text-xs bg-primary text-primary-foreground px-3 py-1.5 rounded-md hover:opacity-90">
                  ...
                  {status === "received" && (
                    <button onClick={() => acceptRequest(profile.user_id)} className="flex items-center gap-1 text-xs bg-secondary text-secondary-foreground px-3 py-1.5 rounded-md hover:opacity-90">
                      <UserCheck className="w-3.5 h-3.5" /> Accept
                    </button>
                  )}
                  {status === "connected" && (
                    <span className="flex items-center gap-1 text-xs text-primary font-semibold"><UserCheck className="w-3.5 h-3.5" /> Connected</span>
                  )}
                </div>
              </div>
            );
